"""Recurring payment execution logic.

The functions here advance due dates and create automatic expense
transactions for active recurring payments or subscriptions.
"""
...
def next_due_date(current_due_date, frequency):
    """Return the next due date for a supported recurring frequency."""
...
def _create_auto_transaction(recurring, due_date):
    """Create the transaction generated by one recurring due date."""
...
def _process_single_recurring(recurring, run_date, max_cycles=24):
    """Process one recurring row up to run_date, bounded by max_cycles."""
    created_count = 0
    skipped_count = 0
    cycles = 0
...
def execute_due_recurrings_for_user(user, run_date=None, is_subscription=None):
    """Execute due recurring payments for one user and return batch stats."""
